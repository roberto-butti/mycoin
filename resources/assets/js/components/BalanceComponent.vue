    ...
    <div class="columns is- is-marginless">
      ...
      <div class="column is-4">
        <table class="table is-striped is-narrow is-fullwidth">
          <thead>
            <tr>
              <th colspan="2">Ticker and Spread</th>
              <th>
          ...
          <tbody>
            <tr class="is-small" v-for="(item, index) in this.tickers">
              ...
              <td>
                <span
                  class="tag"
                  v-bind:class="{ 'is-warning':differenceBidAsk(index)>3 && differenceBidAsk(index)<=5,  'is-success': differenceBidAsk(index)>5 }"
                >{{ differenceBidAsk(index) }}</span>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="column is-4">
        <table class="table is-striped is-narrow is-fullwidth">
          <thead>
            <tr>
              <th colspan="5">Realtime Trade</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
        ...
        <div class="tabs">
          <ul>
            <li v-for="(instrument, index) in this.instruments">
              <a @click.prevent="selectInstrument(instrument)" class="is-small">{{ instrument }}</a>
            </li>
          </ul>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      instruments: ["LTCEUR", "PPCEUR", "ETHEUR", "BTCEUR", "EURXRP"],
      loading: false,
      balances: [],
      sum: 0,
      selected: null,
    ...
    fetchTickers() {
      axios.get("/api/rock/tickers").then(response => {
        ...
        for (let j = 0; j < response.data["tickers"].length; j++) {
          ...
          if (
            this.instruments.indexOf(response.data["tickers"][j].fund_id) >= 0
          ) {
    ...
    subscribe() {
      ...
      pusher.bind("last_trade", data => {
        ...
        if (this.instruments.indexOf(data["symbol"]) >= 0) {
          this.newtrade.unshift(
            data /*{ "symbol": data["symbol"], "value": data["value"]}*/
          );
